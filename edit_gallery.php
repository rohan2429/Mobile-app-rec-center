...
$query ="select * from gu_gallery where galley_id=$galley_id";
$rowCount=mysql_query($query);
 if($rowCount > 0){
while($cat= mysql_fetch_assoc($rowCount)){	 
?>
<input type="hidden" name="galley_id" value="<?=$cat['galley_id']?>">
...
<label>Description</label>
 <input type="text" class="form-control" name="description" value="<?=$cat['description']?>" required />
 </div>
 
<div class="box-footer">
<button type="submit" name="action" value="update_gallery" class="btn btn-primary">Update</button>
  </div>
  <?php
}
 }?>
