...
$query ="select * from gu_special_events where special_event_id=$special_event_id";
$rowCount=mysql_query($query);
 if($rowCount > 0){
while($cat= mysql_fetch_assoc($rowCount)){	 
?>
<input type="hidden" name="special_event_id" value="<?=$special_event_id?>">

 <div class="form-group">
<label>TITLE</label>
 <input type="text" class="form-control" name="event_title" value="<?=$cat['event_title']?>" readonly>
 </div>
  <div class="form-group">
<label>Date</label>
...
<button type="submit" name="action" value="add-special-date" class="btn btn-primary">Submit</button>
  </div> 
</div>
</form>
