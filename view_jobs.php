...
?>
<a href="add-jobs.php?employment_id=<?=$employment_id?>" style="float:right"><button class="btn btn-primary">Add Jobs</button></a>
</div>	
<div class="box-body">
<?php 
...
$employment_id=$_REQUEST['employment_id'];
$query ="select ge.employment_type,gj.job_id,gj.employment_id,
	gj.company_name,gj.job_title,gj.date,gj.address
	from gu_jobs gj ,gu_employement ge where gj.employment_id=ge.employment_id and gj.employment_id=$employment_id" ;
$rowCount=mysql_query($query);
 if($rowCount > 0){
while($cat= mysql_fetch_assoc($rowCount)){	 
?>	 
<tr>
<input type="hidden" name="job_id" value="<?=$cat['job_id']?>">
<td>
<span>
<?=$cat['employment_type']?></span>
</td>
<td>
<span>
<?=$cat['job_title']?></span>
...
<a href="delete_controller.php?action=deleteJobs&job_id=<?=$cat['job_id']?>&employment_id=<?=$cat['employment_id']?>"  class="btn btn-primary">Delete</a>
</td>
</tr>
<?php 
$i++;
