...
Timings
<small><span>Timings Detailes tables</small>
</h1>
<ol class="breadcrumb">
...
?>
<a href="add_timings.php?hours_oper_id=<?=$hours_oper_id?>" style="float:right"><button class="btn btn-primary">Add Timings</button></a>
</div>	
<div class="box-body">
<?php 
...
$hours_oper_id=$_REQUEST['hours_oper_id'];
$query ="select * from hour_oper_timings where hours_oper_id=$hours_oper_id" ;
$rowCount=mysql_query($query);
 if($rowCount > 0){
while($cat= mysql_fetch_assoc($rowCount)){	 
?>	 
<tr>
<input type="hidden" name="timings_id" value="<?=$cat['$timings_id']?>">
...
<a href="delete_controller.php?action=deleteTimings&timings_id=<?=$cat['timings_id']?>&hours_oper_id=<?=$cat['hours_oper_id']?>"  class="btn btn-primary">Delete</a>
