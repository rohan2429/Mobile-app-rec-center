...
$query ="select * from gu_events where event_id=$event_id";
$rowCount=mysql_query($query);
 if($rowCount > 0){
while($cat= mysql_fetch_assoc($rowCount)){	 
?>
<input type="hidden" name="event_id" value="<?=$cat['event_id']?>">

 <div class="form-group">
<label>Catagory List</label>
 <input type="text" class="form-control" name="category_id" value="<?=$cat['category_id']?>" required />
 </div>	
 <div class="form-group">
<label>subcategory list</label>
 <input type="text" class="form-control" name="sub_category_id" value="<?=$cat['sub_category_id']?>" required />
 </div>	
  <div class="form-group">
<label>Event Title</label>
 <input type="text" class="form-control" name="event_title" value="<?=$cat['event_title']?>" required />
 </div>	
  <div class="form-group">
<label>Event Date</label>
 <input type="text" class="form-control" name="event_date" value="<?=$cat['event_date']?>" required />
 </div>
<div class="form-group">
<label>Image</label>
...
<button type="submit" name="action" value="update_event" class="btn btn-primary">Update</button>
  </div>
  <?php
}
 }?>
